...
public class ClientHandler implements Runnable{
    ...
    public ClientHandler(Socket client, HashMap<String, String> translations, EchoServer echoServer)
            throws IOException {
        this.client = client;
    ...
    private void actionSwitch(String action, String data) {
        switch (action) {
            case "ALL" -> actionAll(data);
            case "UPPER" -> pw.println(actionUpper(data));
            case "LOWER" -> pw.println(actionLower(data));
            case "REVERSE" -> pw.println(actionReverse(data));
            case "TRANSLATE" -> pw.println(actionTranslate(data));
        }
    }
